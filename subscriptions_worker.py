...
sentinel = asyncio.run(aioredis.create_sentinel([(REDIS_CONFIG['host'], 26379)]))
...
async def update_subscriptions():
    ...
    for i in range((urls_num - 1) // BATCH_SIZE + 1):
        ...
        for j in range(min(BATCH_SIZE, urls_num - i * BATCH_SIZE)):
            ...
            tasks.append(process_url(url.decode(), session))
...
async def process_url(url, session):
    ...
    url_get = urllib.parse.urljoin(url, 'new/.json?limit=1')
    async with session.get(url_get, params=REDDIT_OAUTH, headers='meme-aggregator-bot') as resp:
    ...
    if post['is_img']:
        ...
        await send_post(subscribers_iter, subscribers_num, bot.send_text_message, post['text'])
...
async def send_post(sub_iter, sub_num, send_function, post):
    ...
    for i in range((sub_num - 1) // BATCH_SIZE + 1):
        ...
        for j in range(min(BATCH_SIZE, sub_num - i * BATCH_SIZE)):
            user_id = await sub_iter.__anext__()
